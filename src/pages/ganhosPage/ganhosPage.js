    ...
    return (
        ...
        <div className="main-despesas">
            ...
            <button className='btn-input' onClick={handleSubmit}>Adicionar ganhos</button>
            </div>

            <div>
            {ganhos.map((ganho, index) => (
                <div key={index} className="despesa-item">
                <DespesaComponent nomeDespesa={ganho.nomeGanho} valorDespesa={ganho.valorGanho} />
                <button className="delete-btn" onClick={() => handleDeleteGanhos(index)}>Apagar</button>
                </div>
            ))}
            </div>
